	...
	async.run(function()
		ui.open_chat_window(function(input)
			...
			vim.schedule(function()
				local ok, err =
					pcall(vim.api.nvim_buf_set_lines, ui.current_buf, -2, -1, false, { "Grok: Reasoning..." })
				if not ok then
				...
				ok, err = pcall(
					...
					ui.current_win,
					{ vim.api.nvim_buf_line_count(ui.current_buf), 0 }
				)
				if not ok then
					...
					if data then
						...
						if json_str == "[DONE]" then
							vim.schedule(function()
								ui.append_response("\n\n")
								local ok, set_err =
									pcall(vim.api.nvim_buf_set_option, ui.current_buf, "modifiable", true)
								if not ok then
						...
						if ok and json.choices and json.choices[1].delta then
							local delta_content = json.choices[1].delta.content
								or json.choices[1].delta.reasoning_content
								or ""
							if delta_content ~= "" then
								...
								vim.schedule(function()
									if is_first_chunk then
										...
										if not set_ok then
											log.error(
												"Failed to set modifiable true for first chunk: "
													.. vim.inspect(set_err)
											)
										end
										...
										local cursor_ok, cursor_err = pcall(
											vim.api.nvim_win_set_cursor,
											ui.current_win,
											{ line_count, #("Grok: " .. delta_content) }
										)
										if not cursor_ok then
											log.error(
												"Failed to set cursor for first chunk: " .. vim.inspect(cursor_err)
											)
										end
										set_ok, set_err =
											pcall(vim.api.nvim_buf_set_option, ui.current_buf, "modifiable", false)
										if not set_ok then
											log.error(
												"Failed to set modifiable false for first chunk: "
													.. vim.inspect(set_err)
											)
										end
									else
		...
		if prompt and prompt ~= "" then
			vim.schedule(function()
				...
				ok, err =
					pcall(vim.api.nvim_buf_set_lines, ui.current_buf, -2, -1, false, { "", "You: " .. prompt, "" })
				if not ok then
