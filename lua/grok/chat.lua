...
local history = {}
M.history = history -- Export for UI re-rendering
  ...
  async.run(function()
    ui.open_chat_window(function(input)
      ...
      vim.schedule(function()
        ...
        ui.protected_end = vim.api.nvim_buf_line_count(ui.current_buf) - 1 -- Update protected (Issue 3)
    ...
    if prompt and prompt ~= "" then
      ...
      vim.schedule(function()
        local ok, err =
          pcall(vim.api.nvim_win_set_cursor, ui.current_win, { vim.api.nvim_buf_line_count(ui.current_win), 0 })
        if not ok then
          log.error("Failed to set cursor: " .. vim.inspect(err))
        end
